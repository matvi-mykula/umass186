...
public class UriCrawlerTest {
	...
	private static final URI LINKED_TO_TRIVIAL_URI = new File(RESOURCE_DIR + "linkedToTrivial.html").getAbsoluteFile()
			.toURI();
	...
	@Test
	public void testVisitAllZero() throws Exception {
		UriCrawler uriCrawler = new UriCrawler(2);
		uriCrawler.visitAll();
	...
	@Test
	public void testVisitAllMaxQuota() throws Exception {
		UriCrawler uriCrawler = new UriCrawler(2);
		uriCrawler.addUri(LINKED_TO_TRIVIAL_URI);
		uriCrawler.addUri(DEADLINK_URI);
		uriCrawler.addUri(DUPLICATE_URI);
		uriCrawler.addUri(TRIVIAL_URI);
		uriCrawler.visitAll();
		...
		RetrievedDocument document = list.get(0);
		assertEquals(LINKED_TO_TRIVIAL_URI, document.uri);
		assertEquals(LINKED_TO_TRIVIAL_CONTENTS, document.text);
		assertEquals(1, document.links.size());
	...
	@Test
	public void testVisitAllDuplicateLinks() throws Exception {
		UriCrawler uriCrawler = new UriCrawler(2);
		uriCrawler.addUri(DUPLICATE_URI);
		uriCrawler.visitAll();
		...
		RetrievedDocument document = set.iterator().next();
		assertEquals(DUPLICATE_URI, document.uri);
		assertEquals(DUPLICATE_CONTENTS, document.text);
		assertEquals(1, document.links.size());
	}

	@Test
	public void testVisitAllDeadLinks() throws Exception {
		UriCrawler uriCrawler = new UriCrawler(2);
		uriCrawler.addUri(LINKED_TO_TRIVIAL_URI);
		uriCrawler.addUri(DEADLINK_URI);
		uriCrawler.visitAll();
